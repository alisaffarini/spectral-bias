...
models more robust to PGD attacks? CLI args for eps, n_seeds, and dataset.
"""
...
def main():
    ...
    if args.dataset != "cifar10":
        tag = f"_{args.dataset}{tag}"
    if args.dataset == "cifar10" and args.eps_num == 8 and not args.out_tag:
        out_path = RES / "recall_adversarial.json"
    else:
        out_path = RES / f"recall_adversarial{tag}.json"
    print(f"[adv-recall dataset={args.dataset} eps={args.eps_num}/255] -> {out_path}", flush=True)
