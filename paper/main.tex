...
\item \textbf{Spectral implicit-bias theorem.} In the eigenbasis of $K$
the kBM flow's $(i,j)$ component evolves at rate $\lambda_i + \lambda_j$;
under a benign-loss assumption this implies a quantitative time-constant
bound for each mode (Theorem~\ref{thm:spectral}). Modes in the bottom
eigenspace of $K$ are unlearnable in the NTK regime.
...
($N \in \{10, 30, 100\}$, depth $L \in \{1, 2, 3\}$) and on Fashion-MNIST
triplets, including a side-by-side comparison of network and ODE
trajectories in $K$'s eigenbasis.
\end{enumerate}
...
with the size of that neighborhood controlled by the bottom eigenvalues
themselves. Section~\ref{sec:experiments} verifies this prediction
numerically and shows that it holds for both networks and the ODE.
...
Figure~\ref{fig:spectral} confirms this prediction. Across two orders of
magnitude in $\lambda_i$, the per-mode displacement is monotonically
related to the eigenvalue, and a power-law fit on the log-log plot gives
slope $\approx 2$ for $L=1$ and $\approx 1.5$ for $L=2$. The bound in
Theorem~\ref{thm:spectral} is linear in $\lambda_i$, so the empirical
slopes are \emph{stronger} than the theorem's worst-case prediction; we
suspect this comes from $\Gtilde_{ii}(0)$ itself being correlated with
$\lambda_i$ at NTK initialization, so the prefactor
$(\Etilde\Gtilde)_{ii}$ in Lemma~\ref{lem:eigen} contributes an additional
$\lambda_i$-dependence that the theorem absorbs into its operator-norm
bound $C$. Sharpening the theorem to recover the empirical scaling is
left for future work; the qualitative prediction --- low-$\lambda$ modes
are essentially frozen relative to high-$\lambda$ modes --- holds across
both depths and all seeds.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{fig_spectral_bias.png}
\caption{Spectral implicit-bias prediction. \textbf{Top:} per-mode
displacement $|\Gtilde_{ii}(T) - \Gtilde_{ii}(0)|$ vs.\ kernel eigenvalue
$\lambda_i$, for depth $L = 1$ and $L = 2$. \textbf{Bottom:} time
trajectories of selected modes; high-$\lambda$ modes evolve fast,
low-$\lambda$ modes are essentially frozen. Mean over 5 seeds.}
\label{fig:spectral}
\end{figure}
