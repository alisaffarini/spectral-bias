...
We state these identities here because they clarify how the
...
parameterisation (\texttt{kbm/network.py}), depth-$L$ NTK kernels via the
Jacot recursion (\texttt{kbm/kernels.py}), RK4 integrator for the kBM ODE
(\texttt{kbm/dynamics.py}), and triplet loss
(\texttt{kbm/loss.py}). Code, configs, and raw multi-seed JSON outputs are
...
The error is largest for the deepest network, consistent with the NTK
approximation being looser for larger $L$ at finite width.}
...
sharpens this to a slope-2 power-law prediction at NTK initialisation.
...
slope is $s = 1.69 \pm 0.03$, in the predicted range. The result transfers
without modification from synthetic to real data
...
mode-wise rate ordering appears as in the synthetic case; fitted slope
$s = 1.69 \pm 0.03$.}
\label{fig:fashion}
\end{figure}
...
the slope-2 spectral bias ($s_v = 2.00 \pm 0.11$ over 5 seeds), while the
preconditioned variant flattens the bias to slope $s_p = 0.74 \pm 0.09$, a
$2.7\times$ reduction (Figure~\ref{fig:precond}). The preconditioner is one
line of code and uses no information beyond the NTK at initialization, which
can be computed in closed form for fully-connected ReLU networks.

\paragraph{Triplet satisfaction.}
On the same synthetic setup, vanilla training achieves $0.83$ triplet
satisfaction and preconditioned training $0.81$. The two methods are tied
on training fit, but they fit \emph{different} triplets: the preconditioner
...
slope $1.59 \pm 0.02$ (mean $\pm$ std over 5 seeds). The slope-2
phenomenon transfers to real image features, and the preconditioner's
flattening effect is preserved.}
\label{fig:cifar}
\end{figure}

The slope $2.06 \pm 0.01$ on real CIFAR-10 features is the closest match
to the slope-2 prediction in any of our experiments, supporting the claim
that the spectral bias is a property of NTK-regime training rather than of
synthetic-data structure. The preconditioner reduces the slope to
$1.59 \pm 0.02$, a $23\%$ reduction in spectral bias on real data
...
spectrum, magnifying the regularised inverse's $\varepsilon = 10^{-2}$
compromise).
...
matrix-solve per training step. The predicted ordering $s_v > s_p$ holds
in every seed.
...
$\norm{\Gtilde(t) - \Gtilde(0)}_F$ on the same trajectory
...
characterizations of the implicit bias miss the spectral phenomenon, and
vice versa.
...
saturates at $N$ across $M \geq N$. Networks under the kBM flow do not
implicitly minimize the nuclear norm of $G$ (or any matrix norm) without
an explicit regularizer; they fit triplets at the constraint-optimal rank.
...
truncate the figure at $\lambda = 30$. Nuclear-norm regularization behaves
as expected on top of the kBM flow; we report this as an empirical
observation rather than a theoretical prediction.
...
empirically. The preconditioner of Section~\ref{sec:exp-precond} is the
algorithmic payoff: it would not have been suggested by the standard NTK
ODE alone.
...
and we verify this empirically in Section~\ref{sec:exp-rank-vs-spec}:
tracking the effective rank of $G(t)$ alongside the spectral-displacement
metric across training, we find vanilla NTK metric learning produces a
...
the regression is dominated by intermediate modes. The end-to-end CNN
result ($s_v = 1.15$) is the largest deviation; this is consistent with
the empirical NTK being a less tight description of finite-width CNN
training than of wide MLP training.
...
\item \emph{Single-layer focus of theory.} Theorems~\ref{thm:kbm}
and~\ref{thm:depth} cover fully-connected ReLU networks; the central
spectral-bias prediction is established in this setting. Convolutional
architectures admit analogous NTK formulas, and our end-to-end CNN
experiment uses the empirical NTK, but we do not give a closed-form
extension of Theorem~\ref{thm:depth} to convolutional or attention
architectures.
\item \emph{Isotropic NTK approximation.} We use the scalar
($K_{nn'}\delta_{mm'}$) NTK approximation. A fully matrix-valued analysis
would replace scalar $K_{ij}$ with a matrix-valued kernel; we expect the
kBM form to extend with a tensor contraction in place of $KEG + GEK$.
\item \emph{Finite sample sizes.} Our empirical evaluation is at $N$ in
the hundreds (real data) and $N \le 1000$ (synthetic), with 5 seeds. The
predicted slope is a leading-order prediction; finite-$N$ deviations from
the asymptotic $s = 2$ are documented and consistent with the bound
$\le 2$.
\item \emph{Loss class.} Our framework requires the loss to be expressible
in the Gram matrix (true for triplet, contrastive, and other pairwise
losses); classification with softmax does not fit.
\item \emph{No generalization bound.} Corollary~\ref{cor:gen} characterises
the effective hypothesis class on bounded horizons but stops short of a
quantitative generalization bound, which would require an additional
analysis of gradient noise and loss-landscape geometry.
\item \emph{Preconditioner is not a free lunch.} Improvement on Recall@$K$
requires that the test geometry actually load on $K$'s bottom eigenspace;
under random-class negatives, where the loss already concentrates on
top-spectrum modes, the preconditioner does not help. The cost is one
extra forward-backward pass per step plus an $O(N^3)$ kernel inversion at
...
intervals. Multi-seed experiments on synthetic data, Fashion-MNIST,
frozen-feature CIFAR-10, and an end-to-end CNN confirm the prediction
across single- and multi-layer networks, and the $K^{-1}$ preconditioner
both flattens the spectral bias and improves hard-negative retrieval.
The kBM lens is, we believe, the right way to read the implicit bias of
metric learning: the rank-bias literature describes a different axis along
which the dynamics is conservative, and the two are complementary.
...
$N=100$ images with triplet loss. The empirical NTK at initialisation is
computed by per-output-feature backward passes
(\texttt{runners/cifar10\_end2end.py}). The preconditioned variant uses
$(K^{\text{emp}} + \varepsilon I)^{-1}$ with $\varepsilon = 10^{-2}$.
